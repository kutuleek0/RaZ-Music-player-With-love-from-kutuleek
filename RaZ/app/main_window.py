...
from . import updater # <-- ДОБАВИТЬ ИМПОРТ
...
class RaZPlayer(ctk.CTk):
    def __init__(self):
        ...
        self.playlist_data = {}; self.current_category = "Все треки"; self.current_track_index = -1
        self.current_song_length = 0; self.is_playing = False; self.is_paused = False
        self.is_shuffle = False; self.is_repeat = False; self.is_recommend_mode = False
        self.is_previewing = False; self.seeking = False; self.pygame = pygame
        ...
        pygame.mixer.init(); ctk.set_appearance_mode("dark")
        self.init_ui()
        ...
        # --- ДОБАВЛЕНО: Запуск проверки обновлений в фоновом потоке ---
    ...
    def init_ui(self):
        ...
        # --- КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Задаем фон и скругление ContentFrame ---
    ...
    # ... (остальной код файла main_window.py остается без изменений) ...
    def apply_theme(self):
        theme_manager.apply_theme(self)
        self.content_frame.refresh_current_view()
    ...
    def add_category(self, new_cat_name):
        if new_cat_name and new_cat_name not in self.playlist_data:
            ...
            self.show_library_view(new_cat_name)
            data_manager.save_playlist(self.playlist_data)
        elif not new_cat_name: messagebox.showwarning("Ошибка", "Имя категории не может быть пустым.")
        else: messagebox.showwarning("Ошибка", f"Категория '{new_cat_name}' уже существует.")
    def delete_category(self, cat_to_delete):
        ...
        if messagebox.askyesno("Подтверждение", f"Вы уверены, что хотите удалить категорию '{cat_to_delete}'?"):
            ...
            self.sidebar.update_playlist_list(list(self.playlist_data.keys()))
            self.sidebar.select_playlist_button("Все треки")
            self.show_library_view("Все треки")
            data_manager.save_playlist(self.playlist_data)
    def add_track(self):
    ...
    def play_track(self, start_time=0):
        ...
        if self.current_track_index == -1 or not self.playlist_data.get(self.current_category): self.stop(); return
        track_info = self.playlist_data[self.current_category][self.current_track_index]
        try:
            pygame.mixer.music.load(track_info['path']); pygame.mixer.music.play(start=start_time)
            if start_time == 0:
                sound = pygame.mixer.Sound(track_info['path']); self.current_song_length = sound.get_length()
        ...
        except pygame.error as e: messagebox.showerror("Ошибка", f"Не удалось воспроизвести: {e}"); self.is_playing = False
    ...
    def play_pause(self):
        if self.is_playing or self.is_previewing:
            if self.is_paused: pygame.mixer.music.unpause(); self.is_paused = False; self.player_bar.update_play_pause_button(True)
            else: pygame.mixer.music.pause(); self.is_paused = True; self.player_bar.update_play_pause_button(False)
        elif self.current_track_index != -1: self.play_track()
    def next_track(self):
    ...
    def update_progress(self):
        if (self.is_playing or self.is_previewing) and not self.is_paused and not self.seeking:
            if self.current_song_length > 0:
                ...
                if current_pos >= 0: self.player_bar.update_progress_slider(current_pos, self.current_song_length)
    ...
    def on_slider_release(self, value):
        ...
        if (self.is_playing or self.is_previewing) and self.current_song_length > 0:
            seek_time = self.current_song_length * (float(value) / 100)
            if self.is_previewing: pygame.mixer.music.set_pos(seek_time)
            else: self.play_track(start_time=seek_time)
