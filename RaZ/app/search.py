...
from . import data_manager # <-- Добавляем импорт

# --- ИЗМЕНЕНО: Используем новый путь ---
...
def download_track(app, track_data, download_btn, preview_btn):
    ...
    ffmpeg_location = "ffmpeg.exe" if os.path.exists("ffmpeg.exe") else "ffmpeg"
    ydl_opts = {'format': 'bestaudio/best', 'outtmpl': filename_template, 'postprocessors': [{'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '192'}], 'ffmpeg_location': ffmpeg_location, 'quiet': True, 'nocheckcertificate': True}
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if os.path.exists(downloaded_file_path):
                ...
                app.after(0, lambda: download_btn.configure(text="✓"))
