...
def apply_theme(app):
    colors = THEMES.get(app.theme_name, list(THEMES.values())[0])
    
    # Обновление градиента фона
    app.bg_frame.update_gradient(colors["gradient_top"], colors["gradient_bottom"])
    app.configure(fg_color=colors["gradient_bottom"])
    
    # Применение темы к панелям. ContentFrame теперь сам управляет своим фоном.
    app.sidebar.apply_theme(colors)
    app.content_frame.apply_theme(colors) # Этот вызов обновит и фон, и внутренние элементы
    app.player_bar.apply_theme(colors)